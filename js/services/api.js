...
async function request(action, payload = {}, { method = 'POST' } = {}) {
  ...
  try {
    ...
    const init = {
      ...
      // Without this, the browser can silently reuse a cached response for
      // the exact same GET URL (which getProducts always is — same action,
      // no params) — so edits made in the Google Sheet never show up on the
      // site until a hard refresh. `t` busts that cache on every call.
      cache: 'no-store',
    };

    if (method === 'GET') {
      const qs = new URLSearchParams({ action, t: Date.now(), ...flatten(payload) }).toString();
      url = `${base}?${qs}`;
    } else {
